...
def back_to_original_pose(robot2):
    """
    ...
    :return:
    """
    distance = Environment2.CARTESIAN_CENTER[2] - robot2.robot.get_current_pose().pose.position.z
    robot2.relative_move(0, 0, distance)


def down_movement(robot2, movement_speed):
    """
    ...
    if not contact_ok:  # If the robot is already in contact with an object, no movement is performed
        waypoints = []
        wpose = robot2.robot.get_current_pose().pose
        wpose.position.z -= (wpose.position.z)  # Third move sideways (z)
        waypoints.append(copy.deepcopy(wpose))

        (plan, fraction) = robot2.robot.move_group.compute_cartesian_path(
            waypoints,  # waypoints to follow
            0.01,  # eef_step
            0.0)  # jump_threshold

        plan = change_plan_speed(plan, movement_speed)
        robot2.robot.move_group.execute(plan, wait=False)
        ...
        robot2.robot.move_group.stop()
        robot2.relative_move(0, 0, 0.001)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        x_move, y_move = robot2.calculate_relative_movement([x_box, y_box])
        # We move the robot to the corner of the box
        robot2.relative_move(x_move, y_move, 0)

        while True:
            communication_problem = down_movement(robot2, 0.2)
            if communication_problem:
                rospy.loginfo("Problem in communications")
            else:
                break

        back_to_original_pose(robot2)
        time.sleep(1)

        x_box, y_box = Environment2.get_relative_corner('ne')
        x_move, y_move = robot2.calculate_relative_movement([x_box, y_box])
        # We move the robot to the corner of the box
        robot2.relative_move(x_move, y_move, 0)
        ...
        x_box, y_box = Environment2.get_relative_corner('nw')
        x_move, y_move = robot2.calculate_relative_movement([x_box, y_box])
        # We move the robot to the corner of the box
        robot2.relative_move(x_move, y_move, 0)
        ...
        x_box, y_box = Environment2.get_relative_corner('sw')
        x_move, y_move = robot2.calculate_relative_movement([x_box, y_box])
        # We move the robot to the corner of the box
        robot2.relative_move(x_move, y_move, 0)
