...
class Environment:
    ...
    def generate_random_state_fall(image=None, strategy='ncc'):
        ...
        def generate_random_coordinates_fall():
            coordinate_x_fall = random.uniform((-Environment.X_LIMIT_FALL + Environment.ENV_BOUNDS_TOLERANCE) / 2,
                                          (Environment.X_LIMIT_FALL - Environment.ENV_BOUNDS_TOLERANCE) / 2)
            coordinate_y_fall = random.uniform((-Environment.Y_LIMIT_FALL + Environment.ENV_BOUNDS_TOLERANCE) / 2,
                                          (Environment.Y_LIMIT_FALL - Environment.ENV_BOUNDS_TOLERANCE) / 2)
        ...
        if strategy == 'ncc' or strategy == 'non_centered_coordinates':
            ...
            blob_detector = BlobDetector(x_length=Environment.X_LENGTH, y_length=Environment.Y_LENGTH, columns=4,
                                         rows=4)
            optimal_quadrant = blob_detector.find_optimal_quadrant(image)
            optimal_point = blob_detector.quadrants_center[optimal_quadrant]
