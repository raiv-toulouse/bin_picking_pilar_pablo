...
class Environment2:
    ...
    def generate_random_state(image=None, strategy='ncc'):
        ...
        def generate_random_coordinates():
            coordinate_x = random.uniform((-Environment2.X_LIMIT + Environment2.ENV_BOUNDS_TOLERANCE) / 2,
                                          (Environment2.X_LIMIT - Environment2.ENV_BOUNDS_TOLERANCE) / 2)
            coordinate_y = random.uniform((-Environment2.Y_LIMIT + Environment2.ENV_BOUNDS_TOLERANCE) / 2,
                                          (Environment2.Y_LIMIT - Environment2.ENV_BOUNDS_TOLERANCE) / 2)
    ...
    def generate_random_state_fall(image=None, strategy='ncc'):
        """
        Calculates random coordinates inside the Relative Environment defined.
        To help the robot empty the box, the generated coordinates won't be in the center of the box, because this is
        the most reachable place of the box.
        ...
        def generate_random_coordinates_fall():
            coordinate_x_fall = random.uniform((-Environment2.X_LIMIT_FALL + Environment2.ENV_BOUNDS_TOLERANCE) / 2,
                                          (Environment2.X_LIMIT_FALL - Environment2.ENV_BOUNDS_TOLERANCE) / 2)
            coordinate_y_fall = random.uniform((-Environment2.Y_LIMIT_FALL + Environment2.ENV_BOUNDS_TOLERANCE) / 2,
                                          (Environment2.Y_LIMIT_FALL - Environment2.ENV_BOUNDS_TOLERANCE) / 2)
        ...
        if strategy == 'ncc' or strategy == 'non_centered_coordinates':
            ...
            while coordinates_in_center:
                ...
                if abs(coordinate_x_fall) > (Environment.X_LIMIT_FALL / 4) or abs(coordinate_y_fall) > (Environment.Y_LIMIT_FALL / 4):
                    coordinates_in_center = False
        elif strategy == 'optimal' and image is not None:  # Before going to a random state, we check that there are pieces in this place
            blob_detector = BlobDetector(x_length=Environment2.X_LENGTH, y_length=Environment2.Y_LENGTH, columns=4,
                                         rows=4)
            optimal_quadrant = blob_detector.find_optimal_quadrant(image)
            optimal_point = blob_detector.quadrants_center[optimal_quadrant]
